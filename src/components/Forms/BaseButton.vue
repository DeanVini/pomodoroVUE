<template>
    ...
    <div v-else-if="type == 'start'">
        <button :class='classes'><h1 class="text-3xl text-center">{{ value }}</h1></button>    
    </div>
    <div v-else-if="type == 'stop'">
        <button :class='`bg-[#e7f8fd] border-[3px] font-bold text-[${color}] border-[${color}] rounded-lg w-[250px] h-[60px] transition-colors duration-400 active:scale-105`'><h1 class="text-3xl text-center">{{ value }}</h1></button>    
    </div>
